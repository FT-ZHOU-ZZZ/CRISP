...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, wandb_handle, header=None):
        i = 0
        if not header:
            header = ""
        ...
        for obj in iterable:
            ...
            if i % print_freq == 0 or i == len(iterable) - 1:
                ...
                if torch.cuda.is_available():
                    message = log_msg.format(i, len(iterable), eta=eta_string, meters=str(self), time=str(iter_time), data=str(data_time), memory=torch.cuda.max_memory_allocated() / MB)
                else:
                    message = log_msg.format(i, len(iterable), eta=eta_string, meters=str(self), time=str(iter_time), data=str(data_time))
                ...
                if self.is_main_process:
                    if self.logger:
                        ...
                        print(message)
                    # parse loss and lr from message
                    pattern = r"loss: (\d+\.\d+) \((\d+\.\d+)\)  lr: (\d+\.\d+) \((\d+\.\d+)\)"
                    match = re.search(pattern, message)
                    loss_current = float(match.group(1))
                    loss_smoothed = float(match.group(2))
                    lr_current = float(match.group(3))
                    lr_smoothed = float(match.group(4))
                    wandb_handle.log({"loss_current": loss_current, "loss_smoothed": loss_smoothed, "lr_current": lr_current, "lr_smoothed": lr_smoothed}, step=i)
        ...
        if self.is_main_process:
            ...
            final_message = "{} Total time: {} ({:.6f} s / it)".format(header, total_time_str, total_time / len(iterable))
            if self.logger:
